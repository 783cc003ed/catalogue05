...
* https://feedly.com/cve/CVE-2025-32718

<blockquote>
 CVE202532718ExploitsSeverityFeedly
</blockquote>
<blockquote>
Integer overflow or wraparound vulnerability in Windows Server Message Block (SMB) protocol that allows an authorized local attacker to exploit a privilege escalation weakness. ... Integer overflow or wraparound in Windows SMB allows an authorized attacker to elevate privileges locally.
</blockquote>
...
<blockquote>
Removing SMB 1.0 protects your systems by eliminating several well known security vulnerabilities. SMB 1.0 lacks the security features of SMB 2.0 and later that help protect against interception. For example, to prevent a compromised connection SMB 3.0 or later uses pre-authentication integrity, encryption, and signing.
</blockquote>
...
* https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-smb/03d05a6f-bbaf-4a9e-a556-036581b02737

<blockquote>
 MSSMBSMBFINDFILEBOTHDIRECTORYINFOExtensions
</blockquote>
<blockquote>
Reserved (1 byte): An 8-bit unsigned integer used to maintain byte alignment. This field MUST be 0x00. ShortName (24 bytes): The 8.3 name for the file formatted as @GMT~XXX where XXX is an array index value in decimal of an array of snapshots returned, starting at an array index value of zero.
</blockquote>
...
<blockquote>
Shares defined in smb.conf take priority over shares of the same name defined in registry. Global smb.conf options stored in registry are used. This can be activated in two different ways: ... The integer parameter specifies the maximum number of threads each smbd process will create when doing parallel asynchronous IO calls. If the number of ...
</blockquote>
...
* https://www.jstor.org/stable/3009018

<blockquote>
 AnIntegerProgrammingApproachtotheVehicleSchedulingProblemJSTOR
</blockquote>
<blockquote>
The integer programme is solved using the Revised Simplex method, ad-ditional constraints being introduced to retain integrality during convergence. The feasible region of this integer programme is initially restricted so that only routes constructed through sets of radially contiguous locations are considered. The effect
</blockquote>
...
* https://link.springer.com/article/10.1057/jors.1976.63

<blockquote>
 AnIntegerProgrammingApproachtotheVehicleSchedulingProblem
</blockquote>
<blockquote>
...
* https://trid.trb.org/view/297076

<blockquote>
 AnIntegerProgrammingApproachtoSchedulingFromtheBookComputer
</blockquote>
<blockquote>
The formulation of crew scheduling as set-partitioning of set-covering integer programs usually gives rise to integer programming problems of excessive size and computational complexity. By imposing sensible additional structure not inherent in the integer programming model, overconstrained integer programming subproblems can be formed. ...
</blockquote>
...
<blockquote>
SMB is a communication protocol to share files over network. This relies on go-smb2 library for communication with SMB protocol. Paths are specified as remote:sharename (or remote: for the lsd command.)
</blockquote>

---

## DuckDuckgo search -> SMB integer overflow
`2025-06-22`
...
<blockquote>
Integer overflow or wraparound in Windows SMB allows an authorized attacker to elevate privileges locally. Published 2025-06-10 17:02:13 Updated 2025-06-12 16:06:39 Source Microsoft Corporation. View at NVD, CVE.org. Vulnerability category: Overflow. Products affected by CVE-2025-32718.
</blockquote>
...
<blockquote>
 SMBCompressionMicrosoftLearn
</blockquote>
<blockquote>
Always require or always reject compression requests. Starting in Windows Server 2022 with update KB5016693 (OS Build 20348.946) and Windows 11 with update KB5016691 (OS Build 22000.918) you can configure an SMB client or SMB server to always request compression and to always reject requests for compression. You can now use Group Policy or PowerShell; in the initial release of Windows 11 and ...
</blockquote>
...
* https://crypto.stackexchange.com/questions/109403/how-does-smb-authentication-work

<blockquote>
 HowdoesSMBauthenticationworkCryptographyStackExchange
</blockquote>
<blockquote>
For SMB 3.0, Ki is the SessionKey. Label - the purpose of this derived key, encoded as string and length for SMB 3.0. Context - the context information of this derived key, encoded as string and length for SMB 3.0. L - An integer that specifies the length of the derived keying material Ko, L is 128 bits for SMB 3.0 cryptographic keys.
</blockquote>
...
<blockquote>
 ManageSMBMultichannelMicrosoftLearn
</blockquote>
<blockquote>
When SMB is deployed with SMB Multichannel, SMB creates multiple TCP/IP connections for a single session with at least one or more connections per interface if the network adapters are RSS-capable. This configuration enables SMB to use the combined network adapter bandwidth that is available and makes it possible for the SMB client to continue ...
</blockquote>
...
<blockquote>
Installing and deploying IBM Storage Scale the SMB protocol with the installation toolkit. Deploying SMB with IBM Storage Scale with the installation toolkit; Using the installation toolkit to perform installation tasks: Explanations and examples. Deploying protocols; Configuring and enabling the SMB protocol services: Configuring and enabling ...
</blockquote>
...
<blockquote>
This results in improved performance and lower overhead compared to traditional SMB implementations that rely on user-space daemons. A denial of service vulnerability has been reported for Linux kernel. This vulnerability is due to an integer underflow in the ksmbd_decode_ntlmssp_auth_blob function.
</blockquote>
...
<blockquote>
Introduction The Server Message Block (SMB) Version 1.0 Protocol defines extensions to the Common Internet File System (CIFS) Protocol, which is specified in [MS-CIFS]. Unless specifically extended or overridden in this document, all specifications and behaviors that are described for Windows NT operating system clients and servers in [MS-CIFS] apply to the Windows client and server ...
</blockquote>
...
<blockquote>
As a defense in depth measure, you can use segmentation and isolation techniques to secure SMB traffic and reduce threats between devices on your network. SMB is used for file sharing, printing, and inter-process communication such as named pipes and RPC. It's also used as a network data fabric for technologies such as Storage Spaces Direct, Storage Replica, Hyper-V Live Migration, and Cluster ...
</blockquote>
...
<blockquote>
Server Message Block Map Network Drive dialog in Windows 10, connecting to a local SMB network drive Server Message Block (SMB) is a communication protocol [1] used to share files, printers, serial ports, and miscellaneous communications between nodes on a network.
</blockquote>
...
* https://github.com/advisories/GHSA-8gqq-cqfg-7f8m

<blockquote>
 IntegeroverfloworwraparoundinWindowsSMBallowsanGitHub
</blockquote>
<blockquote>
Integer overflow or wraparound in Windows SMB allows an... High severity Unreviewed Published 1 hour ago to the GitHub Advisory Database • Updated 1 hour ago
</blockquote>
...
* https://www.wiz.io/vulnerability-database/cve/cve-2025-32718

<blockquote>
 CVE202532718ImpactExploitabilityandMitigationStepsWiz
</blockquote>
<blockquote>
...
* https://vuldb.com/?id.311935

<blockquote>
 CVE202532718MicrosoftWindowsSMBClientintegeroverflow
</blockquote>
<blockquote>
A vulnerability has been found in Microsoft Windows and classified as critical. Affected by this vulnerability is an unknown functionality of the component SMB Client. The manipulation with an unknown input leads to a integer overflow vulnerability. The CWE definition for the vulnerability is CWE-190. The product performs a calculation that can produce an integer overflow or wraparound, when ...
</blockquote>
...
* https://secalerts.co/vulnerability/CVE-2025-32718

<blockquote>
 CVE202532718WindowsSMBClientElevationofPrivilege
</blockquote>
<blockquote>
What type of vulnerability is CVE-2025-32718? CVE-2025-32718 is classified as an elevation of privilege vulnerability due to an integer overflow in Windows SMB. Can CVE-2025-32718 be exploited remotely? No, CVE-2025-32718 requires local access to the system to be exploited.
</blockquote>
...
* https://www.tenable.com/cve/CVE-2025-32718
...
<blockquote>
SMB authentication rate limiter (in the upcoming Server version only) SMB encryption. End-to-end encryption can either be enabled on each SMB share individually or on the entire file server. SMB encryption is controlled by the EncryptData property of both SMB servers. To enable encryption on a particular share (e.g., Projects), use the ...
</blockquote>
...
* https://www.ibm.com/docs/en/aix/7.2.0?topic=management-smb-protocol

<blockquote>
 SMBprotocolIBM
</blockquote>
<blockquote>
The Server Message Block (SMB) protocol is a client-server communication protocol that is used for shared access to files, directories, printers, serial ports, and other resources on a network. It also provides an authenticated inter-process communication (IPC) mechanism. ... Server Message Block file system (SMBFS) allows access to shares on ...
</blockquote>
...
* https://vulners.com/nvd/NVD:CVE-2025-32718

<blockquote>
 CVE202532718vulnerabilitydatabaseVulnerscom
</blockquote>
<blockquote>
Integer overflow in Windows SMB allows local privilege escalation for authorized attackers. Show more. 5 of 5 AI Insights are available for you today. Leverage the power of AI to quickly understand vulnerabilities, impacts, and exploitability. Get AI Insights. Related. Refs. Reporter Title
</blockquote>
...
* https://www.smbaustralia.biz

<blockquote>
 CarTransportSMBAustraliaCarTransportInterstateCar
</blockquote>
<blockquote>
SMB Australia Car Transport has been in business for over 50 years. We can move your vehicles nationally, interstate, and locally! Through our specialised carriers, we can transport your vehicles anywhere within Australia. With a friendly team of drivers and office staff, we are always ready to serv
</blockquote>
...
* https://www.smbaustralia.biz/about

<blockquote>
 AboutSMBAustralia
</blockquote>
<blockquote>
SMB Australia Car Transport has been an Australian owned and family run business for over 55 years. Operations started in 1965 with a single truck moving cars from Sydney to Melbourne and Brisbane! Now we are one of Australia's Largest Independent Car Carriers. We pride ourselves on excellent customer service and competitive rates for ...
</blockquote>
...
<blockquote>
Tips for Connecting to SMB Share Windows 11. Ensure your computer is connected to the same network as the SMB share. Verify that the SMB feature is enabled in Windows Features if you encounter issues. Keep your network credentials handy for a smooth connection process. Use a descriptive drive letter to easily identify the SMB share.
</blockquote>
...
<blockquote>
Server Message Block version 2 and 3. ... Microsoft's [MS-SMB2]: Server Message Block (SMB) Version 2 Protocol Specification. Implementations SMB2 Header Structure. As for the older SMB protocol, all multibyte integers are represented in little-endian format. ... A 64 bit integer that identifies a specific authenticated user on this TCP session.
</blockquote>
...
* https://windowsforum.com/threads/cve-2025-32718-critical-windows-smb-vulnerability-how-to-protect-your-system.369748/

<blockquote>
 CVE202532718CriticalWindowsSMBVulnerabilityHowtoProtect
</blockquote>
<blockquote>
The Windows Server Message Block (SMB) protocol forms the core of many everyday operations: sharing folders on a corporate network, connecting to remote printers, facilitating mapped drives, and much more. ... where improper validation and handling of integer values during SMB packet processing can result in an integer overflow or wraparound ...
</blockquote>
...
<blockquote>
SMB protocol dialects are version-specific implementations of the Server Message Block protocol, negotiated between a client and a server during connection setup. Each dialect corresponds to a particular version of SMB—such as SMB1, SMB2.0, SMB2.1, SMB3.0, SMB3.1.1—and defines the exact features, capabilities, and behavior to be used during ...
</blockquote>
...
<blockquote>
Affected by this vulnerability is an unknown functionality of the component SMB Client. The manipulation with an unknown input leads to a integer overflow vulnerability. The CWE definition for the vulnerability is CWE-190. The product performs a calculation that can produce an integer overflow or wraparound, when the logic assumes that the ...
</blockquote>
...
<blockquote>
Integer overflow or wraparound in Windows SMB allows an authorized attacker to elevate privileges locally. Integer overflow or wraparound in Windows SMB allows an authorized attacker to elevate privileges locally. News. Microsoft Patch Tuesday June 2025. SANS Internet Storm Center / 3min.
</blockquote>
...
<blockquote>
Integer overflow or wraparound in Windows SMB allows an authorized attacker to elevate privileges locally. Integer overflow or wraparound in Windows SMB allows an authorized attacker to elevate privileges locally. CVEs; Settings. Links Tenable Cloud Tenable Community &amp; Support Tenable University. Severity. CVSS v2 CVSS v3 CVSS v4.
</blockquote>
...
<blockquote>
SMB Buffer Overflow Remote Code Execution Vulnerability: MS09-001: RCE: CVE-2008-4835: SMB Validation Remote Code Execution Vulnerability: MS09-001: DOS: ... ETERNALBLUE - Integer overflow SrvOs2FeaListSizeToNt: MS17-010: RCE: CVE-2017-0145??? - Integer overflow SrvOs2GeaListSizeToNt? MS17-010: RCE:
</blockquote>
...
<blockquote>
Integer overflow or wraparound in Windows SMB allows an authorized attacker to elevate privileges locally. Published 2025-06-10 17:02:13 Updated 2025-06-10 17:21:58 Source Microsoft Corporation. View at NVD, CVE.org. Vulnerability category: Overflow. Products affected by CVE-2025-32718.
</blockquote>
...
<blockquote>
Integer overflow or wraparound in Windows SMB allows an authorized attacker to elevate privileges locally. Metrics CVSS Version 4.0 CVSS Version 3.x CVSS Version 2.0 NVD enrichment efforts reference publicly available information to associate vector strings. ...
</blockquote>
...
<blockquote>
Integer overflow or wraparound in Windows SMB allows an authorized attacker to elevate privileges locally. Integer overflow or wraparound in Windows SMB allows an authorized attacker to elevate privileges locally. ... Leverage the power of AI to quickly understand vulnerabilities, impacts, and exploitability. Get AI Insights. Related. Refs ...
</blockquote>
...
<blockquote>
An integer overflow vulnerability is a well-known, yet persistent, challenge in software security. When a calculation involving data received or processed by a program exceeds a variable's capacity, the stored value 'wraps around'—in essence, it begins again from the type's minimum value.
</blockquote>
...
<blockquote>
Vulnerability Details : CVE-2025-32718 Integer overflow or wraparound in Windows SMB allows an authorized attacker to elevate privileges locally. Published 2025-06-10 17:02:13
</blockquote>
...
<blockquote>
 CVE202532718vulnerabilityanalysisandmitigationwizio
</blockquote>
<blockquote>
The vulnerability is classified as an Integer overflow or wraparound vulnerability in Windows SMB, affecting the Windows SMB client component. Microsoft has assigned it two Common Weakness Enumeration (CWE) identifiers: CWE-122 (Heap-based Buffer Overflow) and CWE-190 (Integer Overflow or Wraparound).
</blockquote>
...
* https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2025-32718
...
* https://cvefeed.io/vuln/detail/CVE-2025-32718

<blockquote>
 CVE202532718WindowsSMBIntegerOverflowPrivilegeEscalation
</blockquote>
<blockquote>
Integer overflow or wraparound in Windows SMB allows an authorized attacker to elevate privileges locally. ... Vulnerability history details can be useful for understanding the evolution of a vulnerability, and for identifying the most recent changes that may impact the vulnerability's severity, exploitability, or other characteristics. ...
</blockquote>
...
<blockquote>
 CVE202532718WindowsSMBClientElevationofPrivilegeVulnerability
</blockquote>
<blockquote>
Integer overflow or wraparound in Windows SMB allows an authorized attacker to elevate privileges locally. CVE List News Pricing Book a ... 10/6/2025. 10/6/2025. 13/6/2025. CVE-2025-32718: Windows SMB Client Elevation of Privilege Vulnerability. First published: Tue Jun 10 2025 (Updated:) &lt;p&gt;Integer overflow or wraparound in Windows SMB allows ...
</blockquote>
...
<blockquote>
Retrieves the SMB client directory mappings created for a server. Get-SmbMultichannelConnection: Retrieves the SMB connections made between the SMB client network interfaces and the SMB server network interfaces. Get-SmbMultichannelConstraint: Retrieves the constraints that define how the SMB client uses network interfaces to connect to the ...
</blockquote>
...
<blockquote>
To analyze SMB traffic using Wireshark, you can use display filters to focus on specific SMB versions. For SMBv1, use the filter smb, for SMBv2, use smb2, and for SMBv3, use smb2 &amp;&amp; smb2.cmd &#61;&#61; 0x11. Understanding the differences between SMB versions is crucial for securing your network. To mitigate the risks associated with SMBv1, consider ...
</blockquote>
...
<blockquote>
Exists within the Windows SMB client, not the server implementation. Requires an attacker to be authorized (i.e., have at least a low-privilege account) and have local access. Relies on integer overflow/wraparound conditions, a classic vector for privilege escalation bugs. The Anatomy of an Integer Overflow in Security Context
</blockquote>
...
<blockquote>
 MSSMB2SMB2LOCKRequestMicrosoftLearnlearnmicrosoftcom
</blockquote>
<blockquote>
...
<blockquote>
 IntroductiontoSMBServerMessageBlockProtocolMiniTool
</blockquote>
<blockquote>
...
<blockquote>
SMB2 is a new version of the old Windows filesharing protocol SMB and is used for filesharing on modern and future Windows hosts. Windows 8 introduced several new features, so Microsoft has decided to bump the revision number up to SMB v3. ... The value of this integer is generated by the server upon completion of a successful SMB2/TreeConnect ...
</blockquote>
...
<blockquote>
Server Message Block (SMB) is a communication protocol [1] used to share files, printers, serial ports, and miscellaneous communications between nodes on a network. ... Barry Feigenbaum originally designed SMB at IBM in early 1983 with the aim of turning DOS INT 21h local file access into a networked file system. [3]
</blockquote>
...
<blockquote>
SMB 2.0 (2006). SMB 2.0 was released with Windows Vista in 2006 and Windows Server 2008. It reduced chattiness in SMB 1.0 to improve the protocol's performance and enhance its efficiency, scalability and resiliency. It also added support for WAN acceleration. SMB 2.1 (2010). SMB 2.1 was introduced with Windows Server 2008 R2 and Windows 7.
</blockquote>
...
<blockquote>
KSMBD stands for Kernel-based SMB Direct. It's a Linux kernel module that provides the implementation of the SMBv3 protocol, allowing the Linux kernel to act as a server for SMB (Server Message Block) clients. ... There is an integer underflow exists in the ksmbd kernel module when handling SMB2 SESSION_SETUP messages. Specifically, the flaw ...
</blockquote>
...
* https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-fscc/3668ae46-1df5-4656-b481-763877428bcb

<blockquote>
 MSFSCCFileRenameInformationforSMBMicrosoftLearn
</blockquote>
<blockquote>
RootDirectory (4 bytes): A 32-bit unsigned integer that contains the file handle for the directory to which the new name of the file is relative. For network operations, this value MUST be zero. FileNameLength (4 bytes): A 32-bit unsigned integer that specifies the length, in bytes, of the file name contained within the FileName field.
</blockquote>
