...
* https://techcommunity.microsoft.com/discussions/edgeinsiderdiscussions/no-playready-visible-with-the-chromium-edge/1431587

<blockquote>
 NoPlayReadyvisiblewiththeChromiumEdge
</blockquote>
<blockquote>
I don't know about PlayReady requirements but your hope is that's a missing requirement on the driver side, and thus with an driver update you'll be set. EDIT: I've now seen the screenshot you posted in the other thread and it appears you have Hardware DRM support , but you lack Direct Composition.
</blockquote>
...
<blockquote>
 SolutionFIXCheckerboardlikepatternsrandomlyappearingin
</blockquote>
<blockquote>
This update addresses an issue that affects Direct Composition batched presentations. A brief flash of triangles or boxes show on the screen. This...
</blockquote>
...
* https://issues.chromium.org/issues/329691945

<blockquote>
 DirectcompositionvideoswapchaintestsregressedonnewerAMDdriver
</blockquote>
<blockquote>
On this new driver, a number of direct composition/video swapchain tests are failing when they were passing fine on the older driver. Sample new driver build, sample old driver build
</blockquote>
...
<blockquote>
DirectComposition is a high-performance bitmap composition engine that uses hardware-accelerated graphics to achieve high frame rates, resulting in smooth and consistent panning, scrolling, and animations of complex content.
</blockquote>
...
* https://learn.microsoft.com/en-us/windows-hardware/drivers/usbcon/register-a-composite-driver

<blockquote>
 HowtoRegisteraCompositeDeviceWindowsdrivers
</blockquote>
<blockquote>
The USB driver stack processes the remote wake-up request, which sends the necessary protocol requests to the device. Obtains a list of function handles (one per function) assigned by the USB driver stack. The composite driver can then use a function handle in the driver's the request for remote wake-up of the function associated with the handle.
</blockquote>
...
* https://community.brave.com/t/webgl-disabled-or-unavailable/100676

<blockquote>
 WebGLdisabledorunavailableBrowserSupportBraveCommunity
</blockquote>
<blockquote>
Direct composition path is buggy on certain AMD devices/drivers: [800950] Applied Workarounds: disable_direct_composition; Hardware overlays result in black videos on non-Intel GPUs: [932879] Applied Workarounds: disable_direct_composition_video_overlays
</blockquote>
...
* https://answers.microsoft.com/en-us/windows/forum/all/missing-usb-composite-device-driver-and-how-to/7b666470-47e6-4c3b-a671-4cfbc4c0d9cc

<blockquote>
 MissingUSBCompositeDeviceDriverandHowToDownload
</blockquote>
<blockquote>
For example, when I put on my RGB gaming mouse, the lights turn on, but the mouse does not work. I checked the Universal Serial Bus controllers driver menu from Device Manager. Since the USB Composite Device driver is not visible there (even if the show hidden device feature is turned on), it does not work if there is no correct driver for USB.
</blockquote>
...
<blockquote>
Only adding --disable_direct_composition_video_overlays&#61;1-&gt; not reproduable. So this seems only related to --disable_direct_composition_video_overlays&#61;1 To try to solve this artifact problem, you should completely terminate all Edge (or Chrome) processes, for example, before running them with command line commands.
</blockquote>
...
## DuckDuckgo search -> Direct Composition driver overflow
`2025-06-22`

* https://stackoverflow.org.cn/questions/63381368

<blockquote>
 c直接合成IDCompositionGaussianBlurEffect抛出访问冲突异常StackOverflow中文网
</blockquote>
<blockquote>
有两个问题： 1.dcomp实现似乎很奇怪，因为它成功地回答了这个问题，但 dcompDevice3 实际上以某种方式被破坏，因此崩溃： ComPtr&lt;IDCompositionDevice3&gt; dcompDevice3; HR(dcompDevice-&gt;QueryInterface(__uuidof(IDCompositionDevice), (LPVOID*)&amp;dcompDevice3))
</blockquote>

---

## DuckDuckgo search -> Direct Composition driver vulnerability
`2025-06-22`
...
<blockquote>
An access violation vulnerability exists in the DirectComposition functionality win32kbase.sys driver version 10..22000.593 as part of Windows 11 version 22000.593 and version 10..20348.643 as part of Windows Server 2022 version 20348.643. ... Microsoft DirectComposition is a Windows component that enables high-performance bitmap composition ...
</blockquote>
...
* https://github.com/MicrosoftEdge/WebView2Feedback/issues/888

<blockquote>
 WhenusingatransparentbackgroundthebackgroundisGitHub
</blockquote>
<blockquote>
Hi @aojianwu - you can try passing --disable_direct_composition&#61;0 as an additional browser argument when creating your environment. For your graphics card versions you may experience some degradation in video playback. ... //gpu and verify the value of 'Direct Composition' under 'Driver Information'? If that value is false and transparency is ...
</blockquote>
...
<blockquote>
 Issue1578453003EnableDirectCompositionbydefaultCodeReview
</blockquote>
<blockquote>
Enable DirectComposition by default. Chrome will now use DirectComposition where supported (Win8+ with d3d11 rendering). --disable-direct-composition can be specified to disable it, and a disable_direct_composition driver bug workaround type has been added.
</blockquote>
...
<blockquote>
Exploiting vulnerability in a legacy driver is the same as exploiting any kernel vulnerability. Microsoft introduced a blocklist of known vulnerable drivers to tackle this issue. At the beginning, the list was included into the Windows Defender Application Control feature and was superseded by the Windows Security application in newer Windows ...
</blockquote>
...
<blockquote>
 HowtoinstallthelatestversionofDirectXMicrosoftSupport
</blockquote>
<blockquote>
The DxDiag tool reports detailed information about the DirectX components and drivers installed on your system and can be used. Windows 11 or 10. From Start, type dxdiag in the Search box, and then press enter. Tap or click on dxdiag from the results.
</blockquote>
...
* https://github.com/MicrosoftEdge/WebView2Feedback/issues/1864

<blockquote>
 YourbrowserdoesnotsupportanyoftherequiredgraphicsAPIGitHub
</blockquote>
<blockquote>
Hey @delemojo - it looks like because of some known bugs with the graphics driver you are using, Direct Composition has been disabled, meaning we need to use software rendering. Using software rendering comes with limitations in WebGL. If you want to, you can try the following additional browser args:--ignore-gpu-blocklist --disable-gpu-driver-bug-workarounds
</blockquote>
...
<blockquote>
There is a potential time of check to time of use (TOCTOU) vulnerability when using direct I/O (for IOCTLs or for Read/Write). Be aware that as the driver is accessing the user data buffer, the user app can simultaneously be accessing the same data buffer. ... From alert to driver vulnerability: Microsoft Defender ATP investigation unearths ...
</blockquote>
...
<blockquote>
 BreakingBoundariesInvestigatingVulnerableDriversandMitigating
</blockquote>
<blockquote>
There are rare cases where even an elevation Administrator → Kernel can be considered to be crossing a security boundary with an assigned vulnerability when it is actively being exploited as a zero-day in the wild (e.g., CVE-2024-21338). Even though the decision regarding security boundaries can sometimes be confusing, when it comes to vulnerable drivers, the straightforward boundary between ...
</blockquote>
...
<blockquote>
The fact there's Intel, Lenovo, Dell, and (~4-5 thousand) SIGNED drivers that provide direct access to MSRs and Physical Memory is unacceptable-- it completely negates the need to have a &quot;WHQL&quot; (eg. signed and validated) driver in the first place. Google, &quot;KDMapper&quot; as an example. Reply. HotCakeX. MVP.
</blockquote>
...
<blockquote>
It has nothing to do with direct composition. Because I maniplated Chrome by setting --disable-direct-composition&#61;0 to shortcut and I had direct composition &quot;true&quot;, but still the same issue. New Edge does not inherit the functionalities of the Legacy Edge (at least in my case/PC). New Edge is not supporting Playready on my PC.
</blockquote>
...
<blockquote>
 RenderingonadiscreteGPUusingcrossadapterresources
</blockquote>
<blockquote>
The DWM process opens the cross-adapter resource in the integrated GPU and uses it during composition as a source texture. See the &quot;Composition&quot; operation in the figure. Direct flip presentation model. The Direct3D runtime instructs the discrete GPU's user-mode display driver to create a cross-adapter resource for each swap chain surface.
</blockquote>
...
<blockquote>
Direct Composition Hook Writeup. ... Its less of a graphics pipeline and more of a composition engine that moves around and animates already rendered scenes from a graphics pipeline like DirectX or Direct2D. It is referred to as a bitmap composition engine by Microsoft. ... I created a test driver that hooked all of the functions related to ...
</blockquote>
...
<blockquote>
 cDirectCompositionIDCompositionGaussianBlurEffectThrowing
</blockquote>
<blockquote>
Is there any way to Apply the Blur Effect on to my visual using Direct Composition. The Exception: Exception thrown at 0x6E695565 (dcomp.dll) in DirectComNoCom.exe: 0xC0000005: Access violation reading location 0x41200000. ... (HWND hwnd) { HR(D3D11CreateDevice(nullptr, // Adapter D3D_DRIVER_TYPE_HARDWARE, nullptr, // Module D3D11_CREATE_DEVICE ...
</blockquote>
...
<blockquote>
By adding the --disable-direct-composition launch parameter to the target location of the shortcut for each browser, you solve this issue. I usually have my preferred browser pinned to the taskbar. If there is a desktop shortcut, I right click to unpin the shortcut in the taskbar, then right click the desktop shortcut , go to properties .
</blockquote>
...
<blockquote>
These machines have newer drivers (31..24002.92) than the experimental ones we were testing on before (31..14037.1007). On this new driver, a number of direct composition/video swapchain tests are failing when they were passing fine on the older driver. Sample new driver build, sample old driver build
</blockquote>

---

# GitHub search -> Direct Composition driver
# TOP
