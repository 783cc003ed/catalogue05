...
<blockquote>
Tenda路由器CVE：四个缓冲区溢出漏洞分析复现 snowleopard**** 漏洞分析 9128浏览 · 2021-03-15 07:32
</blockquote>
...
<blockquote>
漏洞名称：Tenda AC9 远程代码执行漏洞 ... 【网络安全】-rce漏洞-pikachu 2034 【网络安全】-文件上传漏洞实战-upload-labs（0~16） 1696 【渗透测试】-vulnhub源码框架漏洞-Os-hackNos-1 1677 【网络安全】-sql 注入进阶 1616; 上一篇： 【工业安全】-CVE-2024-30891- Tenda AC18路由器 命令 ...
</blockquote>
...
* https://ogma.in/understanding-and-mitigating-cve-2025-25507-remote-command-execution-vulnerability-in-tenda-ac6

<blockquote>
 UnderstandingandMitigatingCVE202525507RemoteCommandExecution
</blockquote>
<blockquote>
Understanding CVE-2025-25507: RCE Vulnerability in Tenda AC6. Published in February 2025, CVE-2025-25507 addresses a critical security concern present in the Tenda AC6 15.03.05.16_multi. This vulnerability is classified as a Remote Command Execution (RCE) flaw residing within the formexeCommand function, where the exploitation of the cmdinput parameter could lead to unauthorized execution of ...
</blockquote>
...
<blockquote>
The CVE-2024-46628 vulnerability exposes the Tenda G3 router to remote code execution, posing a significant threat to network security. Attackers can exploit this flaw to execute arbitrary code on the affected system, potentially leading to a complete compromise of the router's functionality. Tenda G3 users are strongly advised to take immediate action to mitigate the […]
</blockquote>
...
* https://dec-solutions.com/cve-2025-45343-tenda-w18e-rce-vulnerability-overview/

<blockquote>
 CVE202545343TendaW18ERCEVulnerabilityOverview
</blockquote>
<blockquote>
In the ever-evolving landscape of cybersecurity, vulnerabilities can pose significant risks to users and organizations alike. One such vulnerability is CVE-2025-45343, which affects the Tenda W18E router. This remote code execution (RCE) flaw allows attackers to execute arbitrary code on vulnerable devices without proper authorization.
</blockquote>
...
* https://www.iotsec-zone.com/article/458

<blockquote>
 TendaAC6命令执行漏洞真机调试分析IOTsecZone
</blockquote>
<blockquote>
AC6_rce_实体调试 品牌：tenda ac6 固件：US_AC6V2.0RTL_V15.03.06.23_multi_TD01 复现漏洞描述：The parameter mac in the function formWriteFacMac is concatenated and causes the execution of a system command. 函数formWriteFacMac中的参数mac拼接命令造成命令执行。
</blockquote>
...
<blockquote>
 UnpatchedTendaWiFiroutervulnerabilitiesleavehomenetworkswide
</blockquote>
<blockquote>
More precisely, if the Tenda AC15 is open to the internet, it is possible to obtain a persistent reverse shell due to the RCE and open telnet daemon. Trivial exploits Home networking equipment traditionally has an indifferent reputation for security, at best. Tenda's issues exemplify wider problems in the product category, ISE told The Daily ...
</blockquote>
...
* https://nvd.nist.gov/vuln/detail/cve-2025-25507
...
<blockquote>
文章浏览阅读1.1k次，点赞11次，收藏11次。本文介绍了腾达路由器的uploadWewifiPic后台命令执行漏洞，详细阐述了漏洞概述、复现环境及复现过程，并提供了默认弱口令信息。同时，给出了厂商的修复建议，包括更新固件和限制访问策略。
</blockquote>
...
<blockquote>
Tenda路由器CVE：四个缓冲区溢出漏洞分析复现 snowleopard**** 漏洞分析 9123浏览 · 2021-03-15 07:32
</blockquote>
...
<blockquote>
近期tenda系列路由器频繁出现漏洞，下面以CVE-2023-27021漏洞为例，简要描述下对于刚入门IOT安全的人来说怎样去挖掘一个路由器的栈溢出漏洞，并深入利用实现RCE 通常我们挖掘固件漏洞，首先要拿到固件，腾达官网提供用于更新的固件包，我们去官网下载固件即可。
</blockquote>
...
<blockquote>
A vulnerability has been found in Tenda AC6 15.03.05.19 and classified as critical. Affected by this vulnerability is the function formWriteFacMac of the file /goform/WriteFacMac of the component API Endpoint. The manipulation of the argument mac leads to command injection. The attack can be launched remotely.
</blockquote>
...
<blockquote>
漏洞的原因是web服务在处理post请求时，对ssid参数直接复制到栈上的一个局部变量中导致栈溢出。 固件版本：US_AC15V1.0BR_V15.03.05.19_multi_TD01
</blockquote>
...
<blockquote>
There is a RCE vulnerability in Tenda AC6 15.03.05.16_multi. In the formexeCommand function, the parameter cmdinput will cause remote command execution. Metrics CVSS Version 4.0 CVSS Version 3.x CVSS Version 2.0 NVD enrichment efforts reference publicly available information to associate vector strings. ...
</blockquote>
...
<blockquote>
PHP EXIF Memory Corruption Exploit to RCE (CVE-2019-9641) Overview A critical vulnerability has been discovered in PHP versions up to 7.1.26, 7.2.15, and 7.3.2, specifically affecting the exif_process_IFD_in_TIFF function within the EXIF component.
</blockquote>
...
* https://cn-sec.com/archives/3042397.html

<blockquote>
 TendaAc8v4RCE漏洞挖掘与分析CNSEC中文网
</blockquote>
<blockquote>
在开始之前，在此任务中不要使用WSL2 / WSL，因为我保证预设网络配置不会工作（就像在MacBook的arm上运行steam一样），尝试使用Ubuntu-22.04 VMware可以为你节省大量时间。 硬件虚拟化。它是一个托管虚拟机监视器：通过动态二进制转换来模拟机器的处理器，并为机器提供一组不同的硬件和设备模型，使其 ...
</blockquote>
...
<blockquote>
 Tenda漏洞环境搭建与复现CVE202010987NOSEC安全讯息平台白帽汇安全研究院
</blockquote>
<blockquote>
作者：lxonz@白帽汇安全研究院 . 影响产品：Tenda系列路由器. 影响固件版本：&lt;V15.03.05.18. 一.环境搭建 1.工具安装. 这个漏洞的环境搭建很坑，坑在了qemu跟宿主机的通信上，这里主要采用qemu用户模式部署的，也尝试了qemu system部署，这两种的部署方法我都会给出对应的做法，大家可以参考着看，由于 ...
</blockquote>
...
<blockquote>
漏洞名称：Tenda AC9 远程代码执行漏洞 ... 【网络安全】-rce漏洞-pikachu 2021 【网络安全】-文件上传漏洞实战-upload-labs（0~16） 1695 【渗透测试】-vulnhub源码框架漏洞-Os-hackNos-1 1669 【网络安全】-sql 注入进阶 1613; 上一篇： 【工业安全】-CVE-2024-30891- Tenda AC18路由器 命令 ...
</blockquote>
...
<blockquote>
路由器通用0day漏洞挖掘及RCE思路 一、概述. 近期tenda系列路由器频繁出现漏洞，下面以CVE-2023-27021漏洞为例，简要描述下对于刚入门IOT安全的人来说怎样去挖掘一个路由器的栈溢出漏洞，并深入利用实现RCE. ... tenda 路由器的web启动程序通常是bin目录下的httpd文件 ...
</blockquote>

---

# GitHub search -> Tenda RCE
