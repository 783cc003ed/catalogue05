## DuckDuckgo search -> NetBios integer overflow
`2025-06-23`

* https://www.semperis.com/blog/ldapnightmare-explained/

<blockquote>
 WhatisLDAPNightmareSemperisAttackCatalog
</blockquote>
<blockquote>
The victim DC broadcasts a NetBIOS name request to resolve the hostname of the malicious host. The malicious host replies with its IP address. ... to the malicious host. The malicious host, controlled by the attacker, provides a response that is crafted to cause an integer overflow in the LDAP client library on the victim DC, causing Windows to ...
</blockquote>
...
<blockquote>
Integer Overflow problem. Ask Question Asked 14 years, 1 month ago. Modified 9 years, 6 months ago. Viewed 15k times 7 . I keep getting a integer overflow problem and i have no idea how to solve it can anyone help? edx conatins 181 and eax contains 174. xor eax,edx mov edx,2 div edx ...
</blockquote>
...
* https://etutorials.org/Networking/network+security+assessment/Chapter+13.+Application-Level+Risks/13.6+Integer+Overflows/

<blockquote>
 136IntegerOverflowsChapter13ApplicationLevelRisks
</blockquote>
<blockquote>
This results in a heap overflow (which can be exploited in a number of ways, depending on the heap implementation). A real-life example of an integer overflow is the challenge-response integer overflow in OpenSSH 3.3 (CVE-2002-0639). Example 13-9 shows the code that is executed when a user requests challenge-response authentication.
</blockquote>
...
* https://www.infosecmatter.com/metasploit-module-library/?mm=auxiliary/dos/samba/read_nttrans_ea_list

<blockquote>
 SambareadnttransealistIntegerOverflowMetasploit
</blockquote>
<blockquote>
...
* https://www.snort.org/rule_docs/1-4657

<blockquote>
 SnortRuleDocs
</blockquote>
<blockquote>
NETBIOS SMB-DS NT Trans NT SET SECURITY DESC unicode SACL overflow attempt. Rule Explanation. Integer overflow in the Samba daemon (smbd) in Samba 2.x and 3.0.x through 3.0.9 allows remote authenticated users to cause a denial of service (application crash) and possibly execute arbitrary code via a Samba request with a large number of security ...
</blockquote>
...
<blockquote>
Rule Category NETBIOS -- Snort has flagged on traffic on the netbios protocol, which is used to share files across a local network. Alert Message NETBIOS SMB-DS NT Trans NT SET SECURITY DESC unicode SACL overflow attempt Rule Explanation Integer overflow in the Samba daemon (smbd) in Samba 2.x and 3.0.x through 3.0.9 allows remote authenticated users to cause a denial of service (application ...
</blockquote>
...
<blockquote>
Windows port 139 (NetBIOS) is most susceptible to this attack. although other services may suffer as well. Rebooting the affected machine is required to resume normal system functioning. /* It is possible to remotely cause denial of service to any windows 95/NT user. It is done by sending OOB [Out Of Band] data to an
</blockquote>
...
<blockquote>
The integer overflow in fun_1 is the only integer overflow defect in the source code above that Coverity will identify using the default options of the INTEGER_OVERFLOW checker.
</blockquote>
...
<blockquote>
The program below has an integer overflow vulnerability and will lead to a buffer overflow. The above code calculates student grades against random numbers of students. If we consider a 32-bit computer architecture, an integer overflow will occur when the value of unsigned integer exceeds 230 - 1.
</blockquote>

---

# GitHub search -> NetBios integer overflow
# MortenSchenkb
