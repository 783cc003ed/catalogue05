## DuckDuckgo search -> win32k integer overflow
`2025-06-23`

* https://www.northit.co.uk/cve/2013/5058

<blockquote>
 Win32kIntegerOverflowVulnerabilitynorthitcouk
</blockquote>
<blockquote>
...
<blockquote>
 PDFFUZZINGTHEWINDOWSKERNELpowerofcommunitynet
</blockquote>
<blockquote>
Win32k.sys win32kfull.sys win32kbase.dll,Ntoskrnl.exe . Something about Fuzzing • What is Fuzzing ? • Automated software testing technique ... Integer Overflow bug, Gain the Kernel R&amp;W via GDI object Use the kernel R&amp;W steal the system token And assign it to Current
</blockquote>
...
<blockquote>
The vulnerable function was win32k!bFill. Below is the difference between the two versions. The diff quickly shows that an integer overflow was fixed, by adding the function UlongMult3, which is used to detect integer overflows by multiplying the supplied two ULONG integers.
</blockquote>
...
* https://zeifan.my/windows-kernel-win32ksys/

<blockquote>
 MicrosoftWindowsKernelWin32ksysIntegerOverflow
</blockquote>
<blockquote>
...
* https://nvd.nist.gov/vuln/detail/CVE-2013-5058
...
<blockquote>
The ntintsafe library provides a set of C functions that perform safe integer arithmetic operations with bounds checking to prevent overflows and underflows in kernel-mode code. ... (STATUS_INTEGER_OVERFLOW). The ntintsafe library has two categories of functions: Conversion functions—These functions perform conversions between two data types.
</blockquote>
...
* https://www.cvedetails.com/microsoft-bulletin/ms13-101/

<blockquote>
 VulnerabilitiesinWindowsKernelModeDriversCouldAllowElevationof
</blockquote>
<blockquote>
Win32k Integer Overflow Vulnerability A denial of service vulnerability exists in the way that the Win32k.sys kernel-mode driver handles objects in memory. An attacker who successfully exploited this vulnerability could cause the target system to stop responding. CVE-2013-5058 Bulletin details at Microsoft.com
</blockquote>
...
* https://stackoverflow.com/questions/74378836/does-win32-interlockedincrement-correctly-support-signed-integer-overflow

<blockquote>
 cDoesWin32InterlockedIncrementcorrectlyStackOverflow
</blockquote>
<blockquote>
While signed integer overflow is undefined according to language rules, a compiler implementation is free to assign well-defined behavior. A &quot;Windows compiler&quot; is well advised to treat signed integer overflow as well defined. What clock do MSG.time and GetMessageTime use? has more information on the topic, for a different context. -
</blockquote>
...
<blockquote>
 MicrosoftWindowsKernelwin32ksysIntegerOverflowMS13101
</blockquote>
<blockquote>
The bug takes place when performing a signed division 'IDIV', the result does not fit in the destination and the kernel raises an 'INTEGER OVERFLOW' exception. 6.1. *Proof of Concept* The following PoC was compiled in VS2012 and tested against Windows XP and Windows 7, and it allows reproducing the vulnerability.
</blockquote>
...
* https://uninformed.org/?v=10&a=2

<blockquote>
 Uninformedvol10article2
</blockquote>
<blockquote>
An integer overflow exists when an arithmetic operation is done between different integers that would go behind maximum integer value and then create a lower integer. As such, the allocated buffer may be smaller than each buffer size which leads to a kernel pool overflow. ... The win32k driver was not built with a secure design and now it ...
</blockquote>
...
* https://github.com/advisories/GHSA-chfc-c374-p43j

<blockquote>
 Integeroverflowinwin32ksysinthekernelmodedriversGitHub
</blockquote>
<blockquote>
...
# GitHub search -> win32k integer overflow
# MortenSchenkb
