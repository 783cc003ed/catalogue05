## DuckDuckgo search -> AFD buffer overflow
`2025-06-23`

* https://blog.cyvitrix.com/buffer-overflow-attacks-understanding-and-preventing-one-of-the-oldest-cyber-threats/

<blockquote>
 BufferOverflowAttacksUnderstandingandPreventingOneoftheOldest
</blockquote>
<blockquote>
...
<blockquote>
 BufferOverflowUnderstandingRisksandPreventionStrategies
</blockquote>
<blockquote>
Types of Buffer Overflow. Buffer overflows can manifest in various forms, each with unique characteristics and exploitation methods. The most common types include: Stack-based Buffer Overflow: This occurs when a program writes more data to a buffer on the stack than it can hold. Stack-based overflows are particularly dangerous as they can ...
</blockquote>
...
<blockquote>
 FindingBufferOverflowwithFuzzing
</blockquote>
<blockquote>
The buffer overflow affected the sudo_warn_gettext_v1 function in some cases. Though this did not immediately crash sudo, in some instances the sudo_warn_gettext_v1 function was executed using the overwritten pointer, causing the crash that got logged by afl. The Solution Fixing Wrappers.
</blockquote>
...
* https://www.vaadata.com/blog/what-is-buffer-overflow-attacks-types-and-security-tips/

<blockquote>
 WhatisBufferOverflowAttacksTypesSecurityTips
</blockquote>
<blockquote>
Buffer overflows fall into two categories: heap-based buffer overflow and stack-based buffer overflow. For the example attacks and exploits below, we use a 32-bit architecture . The binaries have been compiled without modern security mechanisms such as stack canary and ASLR (Address Space Layout Randomization) for ease of exploitation.
</blockquote>
...
* https://github.com/advisories/GHSA-9g67-rqhf-383g

<blockquote>
 BufferoverflowinAutomaticFileDistributorAFD12GitHub
</blockquote>
<blockquote>
...
* https://www.cve.news/cve-2025-21418/

<blockquote>
 CVE202521418DeepDiveIntotheWindowsAncillaryFunctionDriver
</blockquote>
<blockquote>
Security researchers found that AFD.sys handled a particular IOCTL request (a way for user programs to send commands to drivers) without properly checking input. That let local users craft a special request leading to a memory corruption scenario—often a use-after-free or buffer overflow. Exploit Flow Example
</blockquote>
...
<blockquote>
 PDFReverseEngineeringWindowsAFDREcon
</blockquote>
<blockquote>
Windows AFD.sys Steven Vittitoe @bool101 bool@google.com. Outline Why AFD.sys ... HKLM\System\CCS\Services\Afd Buffer sizes DisableRawSecurity - admin raw sockets DefaultSendWindow ... Integer under/overflow issues Signed integer issues. Fuzzing
</blockquote>
...
<blockquote>
 AFD12xWorkingDirectoryLocalBufferOverflowLocalPrivilege
</blockquote>
<blockquote>
AFD 1.2.x - Working Directory Local Buffer Overflow / Local Privilege Escalation. CVE-2002-1503CVE-14542 . local exploit for Unix platform
</blockquote>
...
<blockquote>
AFD is a kernel-mode driver that supports WinSock, a programming interface for accessing network services in Windows. ... The attacker could send a specially crafted IOCTL request to the AFD driver, which would trigger a buffer overflow condition. This could allow the attacker to execute arbitrary code with elevated privileges and potentially ...
</blockquote>
...
* https://offensive.dev/2023/07/from-vulnerability-to-insight-root-cause-analysis-of-cve-2023-28218/

<blockquote>
 FromvulnerabilitytoinsightRootcauseanalysisofCVE202328218
</blockquote>
<blockquote>
On Patch Tuesday for April 2023, one of the vulnerabilities patched was a local privilege escalation within afd.sys (CVE-2023-28218). The Ancillary Function Driver (AFD.sys) component manages all networking from user-mode to kernel-mode and is a very convoluted component. ... Lastly, a heap-based buffer overflow occurs by allowing a size of ...
</blockquote>
...
* https://thesecmaster.com/blog/how-to-fix-cve-2025-21418-a-critical-heap-based-buffer-overflow-vulnerability-in

<blockquote>
 FixCVE202521418WindowsAFDBufferOverflowGuide
</blockquote>
<blockquote>
CVE-2025-21418 stems from a heap-based buffer overflow within AFD.SYS. This occurs when the driver improperly handles the size of data being copied into a buffer allocated on the heap. A local attacker can exploit this by sending a specially crafted input to AFD.SYS via a WinSock application. This crafted input overflows the buffer, allowing ...
</blockquote>
...
* https://www.wiz.io/vulnerability-database/cve/cve-2023-21768

<blockquote>
 CVE202321768ImpactExploitabilityandMitigationStepsWiz
</blockquote>
<blockquote>
The issue specifically relates to how the AFD driver handles user-mode input/output (I/O) operations. An attacker can exploit this by sending a malicious input/output control (IOCTL) request to the AFD driver, potentially triggering a buffer overflow condition that enables arbitrary code execution with elevated privileges.
</blockquote>
...
* https://rstforums.com/forum/topic/89252-cve-2012-0148-a-deep-dive-into-afd/

<blockquote>
 CVE20120148ADeepDiveIntoAFDRomanianSecurityTeam
</blockquote>
<blockquote>
AFD!AfdPoll Integer Overflow Vulnerability (CVE-2012-0148) ... Before processing the user-supplied buffer (AFD_POLL_INFO) to query each individual socket, afd!AfdPoll ensures that the buffer is large enough to fit the number of records indicated by the HandleCount value. If the size is too small, the function returns with an insufficient size ...
</blockquote>
...
* https://wnesecurity.com/cve-2025-21418-microsoft-windows-ancillary-function-driver-for-winsock-heap-based-buffer-overflow-vulnerability/

<blockquote>
 CVE202521418MicrosoftWindowsAncillaryFunctionDriverforWinSock
</blockquote>
<blockquote>
Learn about CVE-2025-21418 Microsoft Windows Ancillary Function Driver for WinSock Heap-Based Buffer Overflow and other vulnerabilities and best practices by subscribing to our newsletter. Newsletter Subscribe to WNE Security's newsletter for the latest cybersecurity best practices, 0-days, and breaking news.
</blockquote>
...
<blockquote>
By providing the AfdPoll64 function with an AFD_POLL_INFO structure and a HandleCount value of 0×6666668, we can cause the size of the internal buffer allocation to overflow and result in a 0xF8 byte allocation.
</blockquote>
...
<blockquote>
Buffer overflow in Automatic File Distributor (AFD) 1.2... High severity Unreviewed Published on Apr 30, 2022 to the GitHub Advisory Database • Updated on Jan 29, 2023
</blockquote>
...
<blockquote>
This problem occurs because of the architectural behavior of the Windows Sockets API and afd.sys. This problem occurs if all the following conditions are true: The Windows Sockets program uses non-blocking sockets. A single send call or WSASend call fills the whole underlying socket send buffer. For example, the program uses the Windows Sockets setsockopt function to change the default socket ...
</blockquote>
...
* https://thecyberpost.com/tools/exploits-cve/ancillary-function-driver-afd-for-winsock-privilege-escalation/

<blockquote>
 AncillaryFunctionDriverAFDForWinsockPrivilegeEscalation
</blockquote>
<blockquote>
Authored by Christophe de la Fuente, b33f, Yarden Shafir, chompie | Site metasploit.com A vulnerability exists in the Windows Ancillary Function Driver for Winsock (afd.sys) can be leveraged by an attacker to escalate privileges to those of NT AUTHORITYSYSTEM. Due to a flaw in AfdNotifyRemoveIoCompletion, it is possible to create an arbitrary kernel Write-Where primitive, which can be used to ...
</blockquote>
...
<blockquote>
Exploitation of CVE-2023-21768 An attacker could exploit the vulnerability in AFD with valid access to a vulnerable Windows system. The attacker could send a specially crafted IOCTL request to the AFD driver, which would trigger a buffer overflow condition.
</blockquote>
...
<blockquote>
Vulnerability Discovery Security researchers found that AFD.sys handled a particular IOCTL request (a way for user programs to send commands to drivers) without properly checking input. That let local users craft a special request leading to a memory corruption scenario—often a use-after-free or buffer overflow. Exploit Flow Example
</blockquote>
...
<blockquote>
Learn about CVE-2025-21418 Microsoft Windows Ancillary Function Driver for WinSock Heap-Based Buffer Overflow and other newly exploited vulnerabilities and new best practices by subscribing to our newsletter.
</blockquote>

---

# GitHub search -> AFD buffer overflow
# MortenSchenkb
