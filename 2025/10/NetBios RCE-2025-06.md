## DuckDuckgo search -> NetBios RCE
`2025-06-23`

* https://github.com/Dan-Duran/pentest-cheat-sheet

<blockquote>
 GitHubDanDuranpentestcheatsheet
</blockquote>
<blockquote>
Contribute to Dan-Duran/pentest-cheat-sheet development by creating an account on GitHub.
</blockquote>
...
<blockquote>
Port 139 The Network Basic Input Output System ** (NetBIOS)** is a software protocol designed to enable applications, PCs, and Desktops within a local area network (LAN) to interact with network hardware and facilitate the transmission of data across the network. The identification and location of software applications operating on a NetBIOS network are achieved through their NetBIOS names ...
</blockquote>
...
* https://www.infosecmatter.com/nessus-plugin-library/?id=156197

<blockquote>
 ApacheLog4ShellRCEdetectionviacallbackcorrelationDirectCheck
</blockquote>
<blockquote>
This page contains detailed information about the Apache Log4Shell RCE detection via callback correlation (Direct Check NetBIOS) Nessus plugin including available exploits and PoCs found on GitHub, in Metasploit or Exploit-DB for verifying of this vulnerability.
</blockquote>
...
<blockquote>
One way to accomplish this is by using the netbios name of your attacking host (no periods). The URI syntax used above to coerce HTTP authentication (swapping in your attacking hostname). LDAP signing/channel-binding must be disabled (this is the default).
</blockquote>
...
<blockquote>
 ApacheLog4ShellRCEdetectionviacallbackcorrelationDirecTenable
</blockquote>
<blockquote>
Apache Log4Shell RCE detection via callback correlation (Direct Check NetBIOS) critical Nessus Plugin ID 156197
</blockquote>
...
<blockquote>
 EnumeratingSMBRPCandNetBIOSforPentestingPorts445135139
</blockquote>
<blockquote>
This post contains various commands and methods for performing enumeration of the SMB, RPC, and NetBIOS services. This article will be expanded upon as time goes on. Using NMAP Scan for popular RCE…
</blockquote>
...
<blockquote>
 FindingandexploitingCVE20187445unauthenticatedRCEinMedium
</blockquote>
<blockquote>
Apparently, sending a NetBIOS session request (message type 0x81) exercises a vulnerable code path in the SMB binary. Let's extract the binary from the router so we can open it in a disassembler.
</blockquote>
...
* https://medium.com/@NE04j/hack-the-box-blue-writeup-e19839f2e510

<blockquote>
 HackTheBoxBlueWriteupOverviewbyneo4jMedium
</blockquote>
<blockquote>
A very common vulnerability MS17-010 Eternal Blue SMB cause RCE (Remote Code Execution) and gain system access. *Open Ports: — Port 135 (msrpc): Microsoft Windows RPC service is running.
</blockquote>
...
<blockquote>
We have SMB, RPC and NetBIOS. Except SMB, other ones are more correlated to communication protocols. By default, I will firstly enumerate the SMB service. ... Mr Robot | RCE via Penelope Shell Handler. Exploiting remote code execution and reverse shell vulnerabilities in the TryHackMe Mr. Robot machine using Penelope and PHP syntax.
</blockquote>
...
* https://www.linkedin.com/posts/skaif009_full-guide-recon-to-rce-via-smb-137-activity-7321837460903079936-hUd-

<blockquote>
 FullGuideRecontoRCEviaSMB137138139445LinkedIn
</blockquote>
<blockquote>
Achieving RCE — Through writable shares: EXE drops, malicious .lnk shortcuts, startup persistence, scheduled tasks, PsExec/WMI tricks. ... Reconnaissance — Identify exposed SMB/NetBIOS ...
</blockquote>
...
<blockquote>
Finding and exploiting CVE-2018-7445 (unauthenticated RCE in MikroTik's RouterOS SMB) maxi. Follow. ... Finally, the remaining 32 bytes are referred to as Illegal NetBIOS names.
</blockquote>
...
<blockquote>
Here is how to run the Apache Log4Shell RCE detection via callback correlation (Direct Check NetBIOS) as a standalone plugin via the Nessus web user interface (https://localhost:8834/):. Click to start a New Scan.; Select Advanced Scan.; Navigate to the Plugins tab.; On the top right corner click to Disable All plugins.; On the left side table select Misc. plugin family.
</blockquote>
...
<blockquote>
 opsxcqexploitCVE20177494GitHub
</blockquote>
<blockquote>
...
<blockquote>
We have SMB, RPC and NetBIOS. Except SMB, other ones are more correlated to communication protocols. By default, I will firstly enumerate the SMB service. ... Moreover, since both of the vulnerability has the same attack vector which was RCE, I have chosen the first one. If there were two, I am glad to apply both of them. 2) Exploitation.
</blockquote>
...
* https://bhavsec.com/posts/mssql-rce/

<blockquote>
 FromMSSQLtoRCEbhavsecportfolioblog
</blockquote>
<blockquote>
From MSSQL to RCE 🚀 ... 996 closed ports PORT STATE SERVICE VERSION 135 / tcp open msrpc Microsoft Windows RPC 139 / tcp open netbios-ssn Microsoft Windows netbios-ssn 445 / tcp open microsoft-ds Windows Server 2019 Standard 17763 microsoft-ds 1433 / tcp open ms-sql-s Microsoft SQL Server 2017 14.00.1000.00; ...
</blockquote>
...
<blockquote>
It would be very strange to see it on another port. Before Windows 2000, SMB happened over NETBIOS, and thus used TCP/UDP 137,138, and 139. But this is rare today. ... RCE vulnerability exploited by the Conficker worm. MS17-010 / CVE-2017-0144 - RCE vulnerability allegedly leaked from the NSA.
</blockquote>
...
<blockquote>
RPC to RCE steps. To begin setup a relay to the LDAPS server for configuring RBCD. ntlmrelayx.py -t ldaps://dc.windomain.local --delegate-access ... One way to accomplish this is by using the netbios name of your attacking host (no periods). The URI syntax used above to coerce HTTP authentication (swapping in your attacking hostname).
</blockquote>
...
<blockquote>
Apache Log4Shell RCE detection via callback correlation (Direct Check NetBIOS) critical Nessus Plugin ID 156197. Language: English.
</blockquote>

---

# GitHub search -> NetBios RCE
